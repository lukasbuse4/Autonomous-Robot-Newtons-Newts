...
void Turn_Right();
void Turn_Left();
int backR_spd = 90; //90 stop for all motors, 0 is fastest 1 directon, 180 fastest opposite) LB 
int backL_spd = 90;
int frontR_spd = 90;
int frontL_spd = 90;
...
//    Stop();
  ...
  {
    int distance = Get_Distance(10, 11);
    int backDistance = 10000;
    boolean findLeft = true;
//    boolean findBack = true;
    ...
    arm_down();
    delay(500);
//    backDistance = Get_Distance(31,33);
...
void navigate_maze()
{
  ...
  {
    //Left_Correction(20);
    //get distance from front ping
    distance = Get_Distance(10, 11);

    //if close to wall then stop moving closer, First Wall of Maze LB  move forward till hits less than 20 cm
    if(distance < 20) 
    {
      Stop();
      break;
  ...
  //find first hole, then reset position
  int drive_back = find_hole(true); //opening in maze
  reset_pos(drive_back);
  ...
  //gets the drive back moves forward through opening then resets position
  drive_back = find_hole(false);
  Move_Forward();
  delay(2000);
  reset_pos(drive_back);
...
//find hole used in maze, with RL used to tell what direction to move first (RL = true is left)
int find_hole(bool RL)
{
  ...
  while(true)
  {
    ...
    //delay 50 then add 50 + 10 + 2 to delay_cntr
    delay(50);
    delay_cntr += 62; 

    //basically makes sure that the opening is really the opening and not a fluke
    if(distance > 40)  //make sure whole robot gets through hole. Or not a crevice. lb 
    {
      cntr++; 
    }
    if(cntr >= 25)
  ...
  Move_Forward(); //go forward lb 
  delay(2500); //makes sure gets throught the hole lb
  cntr = 0;
  return delay_cntr; //return it bc want to go through hole to find 2nd hole possibility lb. send it drive back
}


//Probably need to add back ping sensor sensing
void reset_pos(int drive_back)
{
  Move_Right();
  delay(drive_back);
...
//yeah need to buff this out, currently not relying on
void Find_Salamanders()
{
  Move_Left();
  delay(2000);
...
void Left_Correction(int ideal_distance)
{
  ...
  //Just a thought - CURRENTLY SETUP TO RUN THIS TRAIN OF THOUGHT

  
    int dist = Get_Distance(10, 11);
    if(dist == (ideal_distance - 5))
    {
      Turn_Right();
      delay(100);
      Move_Backwards();
